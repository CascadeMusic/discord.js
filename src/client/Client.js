...
class Client extends BaseClient {
  ...
  /**
   * Checks whether the provided resource has caching enabled
   * @param {string} resource Resource name
   * @returns {boolean}
   */
  doCache(resource) {
    return this.options.cache.includes(resource);
  }
  ...
  sweepUsers(lifetime = 86400) {
    const lifetimeMs = lifetime * 1000;
    this.users.cache.sweep(t => t.id !== this.user.id && (!t.lastMessageID || Date.now() - SnowflakeUtil.deconstruct(t.lastMessageID).timestamp > lifetimeMs));

    for (const guild of this.guilds.cache.values()) {
      guild.members.cache.sweep(t => !this.users.cache.has(t.id));
      guild.presences.cache.sweep(t => !this.users.cache.has(t.id) && !this.doCache("presences"));
    }
  }
  ...
  sweepChannels(lifetime = 86400) {
    const lifetimeMs = lifetime * 1000;
    if (this.doCache("channels")) {
      return;
    }

    const connections = this.voice ? this.voice.connections.map(t => t.channel.id) : [];
    this.channels.cache.sweep(t => !connections.includes(t.id) && (!t.lastMessageID || Date.now() - SnowflakeUtil.deconstruct(t.lastMessageID).timestamp > lifetimeMs));
